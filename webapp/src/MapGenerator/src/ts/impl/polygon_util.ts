...
export default class PolygonUtil {
     ...
     */
    public static subdividePolygon(p: Vector[], minArea: number, cleanDivide=false): Vector[][] {
        ...
        if (area / (perimeter * perimeter) < 0.001) {
            return [];
        }
        ...
        if (!cleanDivide) {
            ...
            try {
                ...
                for (const s of sliced) {
                    divided.push(...PolygonUtil.subdividePolygon(PolygonUtil.polygonArrayToPolygon(s), minArea, cleanDivide));
                }
            ...
            // Calculate midpoint of the longest side and perpendicular bisector
            const midpoint = longestSide[0].clone().add(longestSide[1]).multiplyScalar(0.5);
            const differenceVector = longestSide[0].clone().sub(longestSide[1]);
            const perpVector = new Vector(differenceVector.y, -differenceVector.x)
                .normalize()
                .multiplyScalar(100);

            const bisectStart = midpoint.clone().add(perpVector);
            const bisectEnd = midpoint.clone().sub(perpVector);
            try {
                // Use slicing to divide the polygon
                const slices = PolyK.Slice(PolygonUtil.polygonToPolygonArray(p), bisectStart.x, bisectStart.y, bisectEnd.x, bisectEnd.y);
        
                // Handle small polygons to avoid gaps
                for (const slice of slices) {
                    const subPolygon = PolygonUtil.polygonArrayToPolygon(slice);
                    const subArea = PolygonUtil.calcPolygonArea(subPolygon);
        
                    // Recursively subdivide only if the subpolygon's area is significant
                    if (subArea >= minArea) {
                        divided.push(...PolygonUtil.subdividePolygon(subPolygon, minArea, cleanDivide));
                    } else {
                        divided.push(subPolygon); // Include small polygons as is to prevent gaps
                    }
                }
    ...
    public static lineToJts(line: Vector[]): jsts.geom.LineString {
        const coords = line.map(v => new jsts.geom.Coordinate(v.x, v.y));
        return PolygonUtil.geometryFactory.createLineString(coords);
    }

    public static polygonToJts(polygon: Vector[]): jsts.geom.Polygon {
     ...
     * [ v.x, v.y, v.x, v.y ]...
     */
    public static polygonToPolygonArray(p: Vector[]): number[] {
        const outP: number[] = [];
        for (const v of p) {
            outP.push(v.x);
            outP.push(v.y);
     ...
     * [ v.x, v.y, v.x, v.y ]...
     */
    public static polygonArrayToPolygon(p: number[]): Vector[] {
        const outP = [];
