...
export class GrayscalePalette extends Palette {
    constructor(grayscaleOptions) {
        const { maxBrightness, ...options } = grayscaleOptions
        super(GrayscalePalette._generateSeed(), options)

        this.maxBrightness = maxBrightness || 1.0
    }
    _generateColor(seed, i) {
        let [h, s, l, _] = seed.hsl()
        s = 0
        l = ((Math.sin(i * Math.PI / 20) * 0.5) + 0.5) * this.maxBrightness
    ...
    static _generateSeed() {
        ...
        return chroma.hsl(0, 0, lightness)
    }
}